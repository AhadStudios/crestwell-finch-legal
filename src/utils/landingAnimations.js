...
export function initExpertiseInteractions(root) {
  ...
  root.querySelectorAll(".expertise-row").forEach((row) => {
    const title = row.querySelector(".expertise-row__title");
    const icon = row.querySelector(".expertise-row__icon");
    const desc = row.querySelector(".expertise-row__desc");
    const item = row.closest(".expertise-item");

    const onEnter = () => {
      gsap.to(title, { x: 6, duration: 0.3, ease: "power2.out", overwrite: "auto" });
      ...
      if (desc) gsap.to(desc, { opacity: 0.85, duration: 0.3, overwrite: "auto" });
    };

    const onLeave = () => {
      const isOpen = item?.classList.contains("expertise-item--open");
      gsap.to(title, { x: isOpen ? 6 : 0, duration: 0.3, ease: "power2.out", overwrite: "auto" });
      gsap.to(icon, {
      ...
      if (desc) gsap.to(desc, { opacity: 1, duration: 0.3, overwrite: "auto" });
    };
